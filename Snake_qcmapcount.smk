...
rule all:
    input:
        ...
        #directory(config['mappeddir']),        
        expand(config['mappeddir']+"{sa_spe1}/Aligned.out.sam", sa_spe1=SPECIES1) 
...
rule mapping_species1:
    ...
    output: 
        #directory(config['mappeddir']),
        config['mappeddir']+"{sa_spe1}/Aligned.out.sam"
    message : 
        "  Mapping  {wildcards.sa_spe1}   ::"+species1+"::"
    shell: 
        "STAR --runThreadN {params.sthreads} --quantMode GeneCounts \
         --genomeDir {params.Idir} --sjdbGTFfile {input.gtf} \
         --readFilesCommand gunzip -c --readFilesIn {input.t1},{input.t2} \
